...
### Step one - setting up your own laptop/computer
...
#### Option 2. using code on your own computer {#usethis}

It is possible to use two packages to install the code directly to your computer: {usethis} and {here}.

The {usethis} has a function called `use_course()` that installs the data from a GitHub repository, unzip the files and delete the original zip.

:::{.callout-tip collapse=false appearance='default' icon=true}
## Where does data default save to? (tip)
...
To check or change your working directory you can find information in Tools > Global Options... and the second box, possibly with `~` will be your working directory. 
Clicking on `Browse...` will open an Explorer box where you can see the folders and confirm where this is or change to another folder.
:::

The {here} package is a robust way of navigating folders in code and is particularly useful if you use a VPN with networked drives as setting a working directory can sometimes be tricky if they start with `\\` or have particularly long names as can be found in OneDrive.
...
# To see your directory type

here::here()
```

#### Option 2. using the Cloud

The Cloud workspace is a Project and using {usethis} creates a project so this becomes "nested" so it's better to connect directly from the GitHub repository by clicking  on the blue button for `New Project` in the top right from the main screen.
...
As well as the packages for the course you will also need to install the packages listed in [Option 2. using code on your own computer](#usethis).
Once you have the packages {usethis} and {here} installed run the following code:
